...
function StatCard({ num, label, icon, delay }) {
  ...
  return (
    <div ref={ref} className={`stat-pill gap-1 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
...
/* ─── service card with hover flip overlay ─── */
function ServiceCard({ svc, pick, index }) {
  ...
  return (
    <motion.div
      className="relative bg-white rounded-2xl border border-slate-100 overflow-hidden cursor-pointer"
      initial={{ opacity: 0, y: 40 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, margin: '-60px' }}
      transition={{ duration: 0.5, delay: index * 0.07, ease: [0.22,0.68,0,1.2] }}
    ...
    >
      ...
      <div className="p-6">
        ...
        <h3 className="font-bold text-slate-800 text-lg leading-snug mb-2 group-hover:text-brand-600">
        ...
        <div className="flex justify-between items-center pt-4 border-t border-slate-100">
          ...
          <span className="text-slate-400 text-xs flex items-center gap-1">
            <span>⏱</span>{pick({ en: svc.duration, ar: svc.durationAr })}
          </span>
        </div>
      </div>
      ...
      <AnimatePresence>
        {hovered && (
          <motion.div
            ...
            className={`absolute inset-0 bg-gradient-to-br ${gradient} flex flex-col items-center justify-center gap-4 p-6`}
          >
            ...
            <Link to="/booking"
              className="mt-2 px-6 py-2.5 bg-white text-brand-600 font-bold rounded-xl text-sm hover:bg-slate-50 transition-colors shadow-lg">
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] bg-gradient-to-br from-brand-50 to-teal-50 overflow-hidden">
        ...
        <div className="hidden w-full h-full items-center justify-center text-5xl font-bold text-brand-300">
          {doc.nameEn?.charAt(4)}
        </div>
        {/* Hover bio overlay */}
        <AnimatePresence>
            ...
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
              ...
              <div className="flex flex-wrap gap-1 mb-2">
                {(pick({ en: doc.specialtiesEn, ar: doc.specialtiesAr }) || '').split(',').slice(0,3).map(s => (
                  <span key={s} className="text-[9px] bg-brand-100 text-brand-600 px-2 py-0.5 rounded-full border border-brand-200">
                    {s.trim()}
                  </span>
                ))}
              </div>
...
function TestimonialCard({ t, pick }) {
  return (
    <motion.div
      className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col gap-4 min-w-[300px] max-w-[340px] flex-shrink-0"
      whileHover={{ y: -4, boxShadow: '0 12px 40px rgba(0,102,204,0.12)' }}
      transition={{ duration: 0.2 }}
    >
...
function TrustBadge({ icon, label }) {
  return (
    <motion.div className="flex items-center gap-3 bg-white rounded-2xl px-5 py-3.5 shadow-card border border-slate-100"
  ...
  return (
    <div ref={revealRef} className="overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center pt-20 overflow-hidden bg-gradient-to-b from-slate-50 to-white">
        ...
        {/* Floating teeth SVGs */}
        <motion.div className="absolute top-28 right-[8%] hidden xl:block opacity-20"
          animate={{ y:[0,-14,0], rotate:[-3,3,-3] }} transition={{ duration:5, repeat:Infinity }}>
          <svg width="70" height="70" viewBox="0 0 24 24" fill="none">
            <path d="M12 2C8.5 2 5 5 5 9c0 3 2 5.5 3.5 7L10 21h4l1.5-5C17 14.5 19 12 19 9c0-4-3.5-7-7-7z" fill="#0066CC"/>
          </svg>
        </motion.div>
        <motion.div className="absolute bottom-32 left-[6%] hidden xl:block opacity-15"
          animate={{ y:[0,10,0], rotate:[3,-3,3] }} transition={{ duration:6, repeat:Infinity, delay:1.5 }}>
        ...
        <div className="relative max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-14 items-center py-16">
          ...
          <div className="relative hidden lg:block">
            ...
            <motion.div initial={{ opacity:0, x:40 }} animate={{ opacity:1, x:0 }} transition={{ duration:0.8, delay:0.2 }}
              className="bg-white rounded-3xl shadow-hover overflow-hidden border border-slate-100">
            ...
            {/* Floating stat chips */}
            {[
              { icon:'🏅', label:{ en:'98.7% Implant Success', ar:'98.7% نجاح الزراعة' }, pos:'top-4 -left-16', delay:0.4 },
              { icon:'⭐', label:{ en:'5-Star Patient Rating', ar:'تقييم 5 نجوم' }, pos:'bottom-16 -left-14', delay:0.55 },
              { icon:'🚀', label:{ en:'Same-Day Results', ar:'نتائج فورية' }, pos:'-top-2 right-4', delay:0.65 },
            ].map((chip, i) => (
              <motion.div key={i} initial={{ opacity:0, scale:0.7 }} animate={{ opacity:1, scale:1 }}
                transition={{ duration:0.4, delay:chip.delay }}
                className={`absolute ${chip.pos} bg-white rounded-2xl shadow-hover border border-slate-100 px-4 py-2.5 flex items-center gap-2.5`}>
          ...
          <div className="grid lg:grid-cols-2 gap-14 items-center">
            <div>
              ...
              <p className="text-slate-500 text-sm leading-relaxed mb-8 reveal">
                {pick({ en: 'SmileCraft combines state-of-the-art technology with a deeply human touch. Every procedure is planned with meticulous precision, every smile designed with artistic care.', ar: 'تجمع سمايل كرافت بين أحدث التقنيات واللمسة الإنسانية العميقة. كل إجراء مخطط بدقة متناهية، وكل ابتسامة مصممة برعاية فنية.' })}
              </p>
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <motion.div key={f.en}
                    ...
                    className="flex items-center gap-3 p-3.5 rounded-xl bg-slate-50 hover:bg-brand-50 border border-slate-100 hover:border-brand-200 transition-all duration-200 cursor-default">
            ...
            <div className="space-y-4">
              {[
                ...
                <motion.div key={s.step}
                  ...
                  className="flex gap-4 p-5 bg-white rounded-2xl border border-slate-100 hover:border-brand-200 hover:shadow-card transition-all duration-250 group">
                  <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-brand-50 group-hover:bg-brand-500 flex items-center justify-center text-2xl transition-colors duration-250">
                    {s.icon}
                  </div>
                  <div>
      ...
      <section className="py-24 bg-white relative overflow-hidden">
        <div className="absolute inset-0 opacity-5"
          style={{ backgroundImage: 'radial-gradient(circle at 20% 50%, #0066CC 1px, transparent 1px), radial-gradient(circle at 80% 50%, #0066CC 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
